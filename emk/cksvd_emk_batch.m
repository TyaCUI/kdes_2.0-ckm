...
% each image path
feaPath = fea_params.feapath;
parfor i = 1:length(feaPath)    
    feaSet = loadfea(feaPath{i});    
    imfea(:,i) = cksvd_emk(feaSet.feaSet, basis_params, emk_params);
    if mod(i,10) == 1
       disp(['Current Iteration is: ' num2str(i)]);
    end
end

function feaSet = loadfea(feapath)
feaSet = load(feapath);
